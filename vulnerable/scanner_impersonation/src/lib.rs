...
impl VulnerableRegistry {
    /// Store `admin` as the registry owner. Panics if already initialized.
    ...
    /// Returns the most recent scan result for `contract_address`, or `None`.
    ...
    /// Returns the full scan history for `contract_address`.
