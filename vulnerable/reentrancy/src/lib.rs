...
impl ReentrantVault {
    ...
    /// VULNERABLE: withdraw `amount` for `user`, notifying `notify_id` before updating state.
    ///
    /// # Vulnerability
    /// The external call to `notify_id.on_withdraw` happens before the balance is
    /// decremented. A malicious `notify_id` can call back into `withdraw` while the
    /// original balance is still intact, draining more than the user's actual balance.
    ...
    /// Returns the current vault balance of `user`.
    ...
    /// Returns the total amount withdrawn by `user` (used to detect over-withdrawal in tests).
...
#[contractimpl]
impl NotifyContract {
    /// Configure the notify contract with vault address and re-entry flag.
    ...
    /// Callback invoked by the vault during withdrawal. Re-enters if configured to do so.
...
mod tests {
    ...
    fn test_secure_reentrant_withdraw_blocks_the_attack() {
        ...
        // Secure contract updates state before external call — reentrant withdraw panics
        vault_client.withdraw(&alice, &1000, &notify_id);
    }
}
